...
def strip_thinking_tags(text: str) -> str:
    ...
    - <invoke ...>...</invoke> - Anthropic tool invoke
    - </thinking> - 残留的闭合标签
    """
    if not text:
        return text
...
def strip_tool_simulation_text(text: str) -> str:
    ...
    # 模拟工具调用的常见模式
    pattern1 = r"^[a-z_]+\s*\([^)]*\)\s*$"  # function_name(args)
    pattern2 = r"^[a-z_]+:\d+[\{\(].*[\}\)]\s*$"  # tool:1{...}
    pattern3 = r'^\{["\']?(tool|function|name)["\']?\s*:'  # JSON-like tool call
...
class ResponseHandler:
    ...
    async def verify_task_completion(
        ...
        # Plan 步骤检查
        try:
            from deskflow.core.plan import get_plan_handler_for_session, has_active_plan
...
NEXT: 建议的下一步"""
    ...
    async def do_task_retrospect(self, task_monitor: Any) -> str:
        ...
        """
        try:
            from deskflow.core.task_monitor import RETROSPECT_PROMPT
            ...
            if self._memory_manager and any(kw in result for kw in ("重复", "无效", "弯路")):
                try:
                    from deskflow.memory.types import Memory, MemoryPriority, MemoryType
    ...
    async def do_task_retrospect_background(
        ...
        try:
            ...
            from deskflow.core.task_monitor import RetrospectRecord, get_retrospect_storage
